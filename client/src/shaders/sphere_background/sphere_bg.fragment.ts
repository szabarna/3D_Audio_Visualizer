...
export const sphereBgFragment = glsl`
  ...
  float cnoise(vec3 P){
   ...
   void main() {
      ...
      vec3 c2 = vec3(0.1, 0.3, 0.75);

      vec3 c3 = vec3(0.0, 0.0, 0.0);
      vec3 c4 = vec3(uColor1) * (vDist + 0.1);
  
      float disc = 1.0 - smoothstep(-0.2, 0.5, length(gl_PointCoord - vec2(0.5) ));

      // vec4 textureOne = texture2D(uTexture, vUv + vec2(vDist) );
      // vec3 mixed = mix(c3, colorsFromTexture + (vDist * 0.1), vColorProgress + 0.1);

      vec4 textureOne = texture2D(uTexture, vUv + (vec2(vDist * 1.15) ) );

      vec3 colorsFromTexture = textureOne.rgb;

      // mixed w/ texture colors
       vec3 mixed = mix(c3, c4, vColorProgress + 0.1);
      // + (vDist * 0.075)
    ...
    // gl_FragColor = vec4(vec3(dist), disc);
     gl_FragColor = vec4(colorsFromTexture, disc);
